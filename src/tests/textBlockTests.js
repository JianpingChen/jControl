...
{
	var body = j.body();
test('textBlock should have DIV tag', function(){
	var initial = 'This is a textBlock';
	var t = j.textBlock(initial);
	equal(t.getTagName(), 'DIV');
	equal(t.getTextContent(), initial);
});

test('can set textBlock content', function(){
	var t = j.textBlock('This is a textBlock');
	var c = 'message';
	t.setTextContent(c);
